...
module Spree
  class Calculator::FlatPercentItemTotal < Calculator
    ...
    def compute(object)
      ...
      current_amount = object.running_total ? (object.amount - object.running_total.abs) : object.amount
      computed_amount = (current_amount * preferred_flat_percent / 100).round(currency_exponent)

      # We don't want to cause the promotion adjustments to push the order into a negative total.
      if computed_amount > current_amount
        current_amount
      else
        computed_amount
      end
